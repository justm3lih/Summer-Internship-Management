...
import { coordinatorReviewSummerLetter, getCoordinatorPendingSummerLetters } from "@/lib/api";
...
export default function CoordinatorSummerLettersPage() {
  ...
  const load = () => {
    setLoading(true);
    getCoordinatorPendingSummerLetters().then((list) => {
      setRows(list);
      setLoading(false);
    });
  };
  ...
  useEffect(() => {
    let cancelled = false;
    void getCoordinatorPendingSummerLetters().then((list) => {
      if (!cancelled) {
        setRows(list);
        setLoading(false);
      }
  ...
  const review = async (id: string, approve: boolean) => {
    ...
    const res = await coordinatorReviewSummerLetter(id, approve, comments || undefined);
    setBusyId(null);
    if (!res.success) {
      showToast(res.message, "error");
      return;
    }
    showToast(approve ? "Letter approved. Student may apply." : "Returned to student.", "success");
    load();
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Letters advisor-approved for this period. Review the course table before final approval."
      />
      ...
      {loading ? (
        ...
        <p className="text-sm text-muted-foreground">Nothing in the coordinator queue right now.</p>
      ) : (
          ...
          {rows.map((row) => (
            <Card key={row.id}>
              <CardHeader>
                ...
                <CardDescription>
                  Advisor:{" "}
                  {row.student?.advisorName?.trim() ||
                    `${row.student?.department ?? ""}`.trim() ||
                    "—"}
                  <br />
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <p className="text-xs font-medium text-muted-foreground">Course table</p>
